...
- **完全解耦**: 严禁使用浏览器原生弹窗（`alert`, `confirm`, `prompt`），必须使用内置 React 模态框组件。这确保了在所有 WebView 或沙盒环境中的一致性。
- **状态反馈**: 按钮点击后若有异步操作，必须显示 Loading 状态并禁用按钮。
- **破坏性操作**: 删除、覆盖、交卷等操作必须进行二次确认。

## 6. 常见反模式 (Bad Practices)
